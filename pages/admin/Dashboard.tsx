...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-stone-100 flex">
      {/* Sidebar */}
      <aside className="w-64 bg-stone-900 text-white fixed h-full hidden md:block">
        <div className="p-8">
          <h2 className="font-royal text-xl tracking-widest mb-12">KAKATIYAS <span className="text-xs block text-stone-500 mt-2 font-sans">Admin Portal</span></h2>
          <nav className="space-y-4">
            {['ANALYTICS', 'PRODUCTS', 'ORDERS', 'ARTISANS', 'SETTINGS'].map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab as any)}
                className={`w-full text-left px-4 py-3 text-xs uppercase tracking-widest transition-colors ${
                  activeTab === tab ? 'bg-kakatiya-gold text-white' : 'text-stone-400 hover:text-white'
                }`}
              >
                {tab}
              </button>
      ...
      {/* Main Content */}
      <main className="md:ml-64 flex-1 p-8 md:p-12 overflow-y-auto">
        <header className="flex justify-between items-center mb-12">
          <h1 className="font-royal text-3xl text-stone-800 capitalize">{activeTab.toLowerCase()}</h1>
